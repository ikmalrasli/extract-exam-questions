...
- **Answer Space**
    ...
    **DO NOT COPY** exactly from these references when generating JSON output, **ONLY study** the reference PDF and the output structure."""),
    ...
    def extract_questions(self, pdf: bytes, ranges: List[Tuple[int, int]]) -> List[dict]:
                ...
                try:
                    ...
                    response = self.chat.send_message(
                        message= types.Content(
                            ...
                            parts=[
                                types.Part.from_bytes(data=pdf, mime_type='application/pdf'),
                                types.Part.from_text(text=f"Extract ONLY **Main Questions {start} to {end}** from the provided PDF."),
                            ]
                        ),
